...
public class ShellBolt implements Bolt {
	...
	public void execute(List<Object> value) {
		...
		while (isRunning && !isSuspended) {
			if (exception != null) {
				LOG.error("shell bolt error occur", exception);
	...
	public void startReaderAndWriter() {
		// reader
		readerThread = new Thread(new BoltReaderRunnable(), "BoltReader");
		readerThread.start();

		// writer
		writerThread = new Thread(new BoltWriterRunnable(), "BoltWriter");
		writerThread.start();
	}
		...
		public void run() {
			while (isRunning) {
				try {
					...
					LOG.error("BoltWriter InterruptedException occurred", e);
					die(e);
					return;
				} catch (Throwable t) {
