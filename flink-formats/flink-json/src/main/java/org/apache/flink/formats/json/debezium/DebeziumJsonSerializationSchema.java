...
public class DebeziumJsonSerializationSchema implements SerializationSchema<RowData> {
	...
	public DebeziumJsonSerializationSchema(
		...
		jsonSerializer = JsonRowDataSerializationSchema.builder()
			.setRowType(createJsonRowType(rowType))
	...
	private static RowType createJsonRowType(RowType databaseSchema) {
		// Debezium JSON contains some other information, e.g. "source", "ts_ms"
		// but we don't need them.
		DataType dataType = LogicalTypeDataTypeConverter.toDataType(databaseSchema);
		return (RowType)
				DataTypes.ROW(
