...
public class HttpUtil {
	...
	public static org.apache.flink.monitor.utils.HttpUtil.HttpResponsePojo sendPost(String url, String jsonStr, Map<String, String> headers)
		throws IOException {
	...
	public static org.apache.flink.monitor.utils.HttpUtil.HttpResponsePojo sendGet(String url) throws IOException {
		return sendGet(url, null);
	}

	public static org.apache.flink.monitor.utils.HttpUtil.HttpResponsePojo sendGet(String url, Map<String, String> headers) throws IOException {
	...
	public static org.apache.flink.monitor.utils.HttpUtil.HttpResponsePojo sendRequest(HttpUriRequest request) throws IOException {
		try (CloseableHttpClient closeableHttpClient = HttpClients.custom().setDefaultRequestConfig(RequestConfig.copy(requestConfig).build()).build();
			CloseableHttpResponse response = closeableHttpClient.execute(request)) {
			int statusCode = response.getStatusLine().getStatusCode();
			String resStr = EntityUtils.toString(response.getEntity(), "UTF-8");
			return new org.apache.flink.monitor.utils.HttpUtil.HttpResponsePojo(statusCode, resStr);
		}
	}
