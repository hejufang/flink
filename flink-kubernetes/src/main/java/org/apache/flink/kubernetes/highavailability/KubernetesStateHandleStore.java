 ...
 * amount of data in ConfigMap small. ConfigMap is build for data less than 1MB whereas
 * state can grow to multiple MBs and GBs.
 *
 * <p>This is a very different implementation with {@link org.apache.flink.runtime.zookeeper.ZooKeeperStateHandleStore}.
 * Benefit from the {@link FlinkKubeClient#checkAndUpdateConfigMap} transactional operation, we could guarantee that
 * only the leader could update the store. Then we will completely get rid of the lock-and-release in Zookeeper
 * implementation.
 *
...
public class KubernetesStateHandleStore<T extends Serializable> implements StateHandleStore<T, StringResourceVersion> {
	...
	/**
	 * Creates a state handle, stores it in ConfigMap. We could guarantee that only the leader could update the
	 * ConfigMap. Since “Get(check the leader)-and-Update(write back to the ConfigMap)” is a
	...
	/**
	 * Replaces a state handle in ConfigMap and discards the old state handle.  Wo do not lock resource
	 * version and then replace in Kubernetes. Since the ConfigMap is periodically updated by leader, the
	 * resource version changes very fast. We use a "check-existence and update" transactional operation instead.
	 *
	 * @param key Key in ConfigMap
	...
	public boolean delete(String name) throws Exception {
		// TODO @chenzhanghao.1997: support delete operation used by delete checkpoint
		return false;
	}
