...
public class KubernetesJobManagerFactoryTest extends KubernetesJobManagerTestBase {
	...
	@Test
	public void testApplicationArceeNotEnabled() {
		final ArceeApplication resultApplication = this.kubernetesJobManagerSpecification.getApplication();
	...
	public void testApplicationMetadataAndSpecFromDeployment() throws IOException {
		...
		final ArceeApplication resultApplication = kubernetesJobManagerSpecification.getApplication();
		assertEquals(KubernetesUtils.getDeploymentName(CLUSTER_ID), resultApplication.getMetadata().getName());
		final Map<String, String> expectedLabels = getCommonLabels();
		expectedLabels.put(Constants.LABEL_COMPONENT_KEY, Constants.LABEL_COMPONENT_JOB_MANAGER);
		expectedLabels.putAll(userLabels);
		...
		assertEquals(expectedLabels, resultApplication.getSpec().getAmSpec().getPodSpec().getMetadata().getLabels());
	...
	public void testApplicationSpecFromConfiguration() throws IOException {
		...
		flinkConfig.set(KubernetesConfigOptions.ARCEE_ADMISSION_CONFIG_ACCOUNT, "test_arcee_account");
		...
		flinkConfig.set(KubernetesConfigOptions.ARCEE_SCHEDULING_CONFIG_SCHEDULE_TIMEOUT_SECONDS, 130);
		...
		assertEquals(resultApplicationSpec.getRestartPolicy().getType(), RestartPolicyType.OnFailure);
		...
		assertEquals(resultApplicationSpec.getSchedulingConfig().getScheduleTimeoutSeconds().longValue(), 130);
	}

	@Test
	public void testApplicationSpecFromCommonConfiguration() throws IOException {
		flinkConfig.set(KubernetesConfigOptions.ARCEE_ENABLED, true);
		flinkConfig.set(PipelineOptions.NAME, "test_app_name");
		flinkConfig.setString(ConfigConstants.OWNER_KEY, "test_arcee_user");
		flinkConfig.setString(ConfigConstants.QUEUE_KEY, "test_arcee_queue");
