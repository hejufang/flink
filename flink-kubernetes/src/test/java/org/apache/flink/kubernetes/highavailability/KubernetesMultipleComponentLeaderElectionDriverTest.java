...
import org.apache.flink.runtime.leaderelection.AbstractLeaderElectionEvent;
		...
		new TestFixture() {
			{
				runTest(
					() -> {
						...
						leaderElectionListener.await(AbstractLeaderElectionEvent.IsLeaderEvent.class);
					});
			}
		};
	}
		...
		new TestFixture() {
			{
				runTest(
					() -> {
						...
						leaderElectionListener.await(AbstractLeaderElectionEvent.NotLeaderEvent.class);
					});
			}
		};
	}
		...
		new TestFixture() {
			{
				runTest(
					() -> {
						...
						final AbstractLeaderElectionEvent.AllKnownLeaderInformationEvent
							allKnownLeaderInformationEvent =
							leaderElectionListener.await(
								AbstractLeaderElectionEvent
