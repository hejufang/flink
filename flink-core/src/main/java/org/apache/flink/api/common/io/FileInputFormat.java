...
public abstract class FileInputFormat<OT> extends RichInputFormat<OT, FileInputSplit> {
	...
	private static void initDefaultInflaterInputStreamFactories() {
		InflaterInputStreamFactory<?>[] defaultFactories = {
				...
				SnappyHadoopInputStreamFactory.getInstance(),
		};
	...
	public FileInputSplit[] createInputSplitsParallel(List<FileStatus> files, long totalLength, int minNumSplits) throws IOException {
		...
		if (unsplittable) {
			...
			final long maxSplitSize = totalLength / minNumSplits + (totalLength % minNumSplits == 0 ? 0 : 1);
			return buildSplitsForSplittableInput(files, minNumSplits, maxSplitSize);
		}
	}

	private FileInputSplit[] buildSplitsForUnsplittableInput(List<FileStatus> files, int minNumSplits) throws IOException {
	...
	private FileInputSplit[] buildSplitsForSplittableInput(List<FileStatus> files, int minNumSplits, long maxSplitSize) throws IOException {
		long startMS = System.currentTimeMillis();
