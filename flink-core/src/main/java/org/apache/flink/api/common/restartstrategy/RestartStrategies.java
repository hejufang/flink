...
public class RestartStrategies {
	...
	/**
	 * Generates a AggregatedFailureRateRestartStrategyConfiguration.
	 *
	 * @param failureRate Maximum number of restarts in given interval {@code failureInterval} before failing a job
	 * @param failureInterval Time interval for failures
	 * @param delayInterval Delay in-between restart attempts
	 */
	public static AggregatedFailureRateRestartStrategyConfiguration aggregatedFailureRateRestart(
			int failureRate, Time failureInterval, Time delayInterval) {
		return new AggregatedFailureRateRestartStrategyConfiguration(failureRate, failureInterval, delayInterval);
	}
	...
	/**
	 * Configuration representing a aggregated failure rate restart strategy.
	 */
	public static final class AggregatedFailureRateRestartStrategyConfiguration extends RestartStrategyConfiguration {
		private static final long serialVersionUID = -3990465984150093646L;
		private final int maxFailureRate;

		private final Time failureInterval;
		private final Time delayBetweenAttemptsInterval;

		public AggregatedFailureRateRestartStrategyConfiguration(
				int maxFailureRate,
				Time failureInterval,
				Time delayBetweenAttemptsInterval) {
			this.maxFailureRate = maxFailureRate;
		...
		public String getDescription() {
			return String.format(
					"Aggregated Failure rate restart with maximum of %d failures within interval %s and fixed delay %s.",
					maxFailureRate,
					failureInterval.toString(),
					delayBetweenAttemptsInterval.toString());
		}
		...
		public boolean equals(Object o) {
			...
			AggregatedFailureRateRestartStrategyConfiguration that = (AggregatedFailureRateRestartStrategyConfiguration) o;
			return maxFailureRate == that.maxFailureRate &&
					Objects.equals(failureInterval, that.failureInterval) &&
					Objects.equals(delayBetweenAttemptsInterval, that.delayBetweenAttemptsInterval);
		}
	...
	private static RestartStrategyConfiguration parseConfiguration(
		...
		switch (restartstrategyKind.toLowerCase()) {
			...
			case "aggregated-failure-rate":
				int maxAggregatedFailures = configuration.get(RestartStrategyOptions.RESTART_STRATEGY_AGGREGATED_FAILURE_RATE_MAX_FAILURES_PER_INTERVAL);
				Duration aggregatedFailureRateInterval = configuration.get(RestartStrategyOptions.RESTART_STRATEGY_AGGREGATED_FAILURE_RATE_FAILURE_RATE_INTERVAL);
				Duration aggregatedFailureRateDelay = configuration.get(RestartStrategyOptions.RESTART_STRATEGY_AGGREGATED_FAILURE_RATE_DELAY);
				return aggregatedFailureRateRestart(
