...
public class CopyExecutionTest {
	...
	public void testIncompleteSplits() throws Exception {
		...
		final ExecutionGraph executionGraph = new ExecutionGraphTestUtils.TestingExecutionGraphBuilder(jobVertex)
				...
				.setSlotProvider(new SimpleSlotProvider(jobId, 2))
		...
		Assert.assertSame(vertex1.getMainExecution().getAttemptNumber(), copyExecution.getAttemptNumber() + 1);
		Assert.assertSame(vertex1.getMainExecution().getState(), ExecutionState.CREATED);
