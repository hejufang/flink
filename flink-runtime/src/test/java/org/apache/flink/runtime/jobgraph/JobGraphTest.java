...
public class JobGraphTest extends TestLogger {
	...
	@Test
	public void testChangeJobVertexBounded() {
		// (batch source) --> op1
		//                       \
		//  		              +-> op3 -> op4
		//                       /
		...
		try {
			JobVertex batchSource = new JobVertex("batchSource");
			JobVertex streamSource = new JobVertex("streamSource");

			JobVertex op1 = new JobVertex("op1");
			JobVertex op2 = new JobVertex("op2");
			JobVertex op3 = new JobVertex("op3");
			JobVertex op4 = new JobVertex("op4");

			op1.connectNewDataSetAsInput(batchSource, DistributionPattern.POINTWISE, ResultPartitionType.PIPELINED);
			op2.connectNewDataSetAsInput(streamSource, DistributionPattern.POINTWISE, ResultPartitionType.PIPELINED);
			op3.connectNewDataSetAsInput(op1, DistributionPattern.POINTWISE, ResultPartitionType.PIPELINED);
			op3.connectNewDataSetAsInput(op2, DistributionPattern.POINTWISE, ResultPartitionType.PIPELINED);
			op4.connectNewDataSetAsInput(op3, DistributionPattern.POINTWISE, ResultPartitionType.PIPELINED);

			JobGraph graph = new JobGraph("TestGraph", batchSource, streamSource, op1, op2, op3, op4);
			graph.setJobVertexBoundedProperties(Collections.singletonList(Tuple2.of(batchSource.getID(), true)));

			assertEquals(streamSource.isBounded(), false);
			assertEquals(batchSource.isBounded(), true);
			assertEquals(op1.isBounded(), true);
			assertEquals(op2.isBounded(), false);
			assertEquals(op3.isBounded(), false);
			assertEquals(op4.isBounded(), false);
		}
		catch (Exception e) {
			e.printStackTrace();
			fail(e.getMessage());
