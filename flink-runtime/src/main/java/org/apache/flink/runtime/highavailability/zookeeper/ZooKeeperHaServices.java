...
public class ZooKeeperHaServices extends AbstractZooKeeperHaServices {
	...
	public ZooKeeperHaServices(
		...
		throws IOException {
		super(curatorFramework, executor, configuration, blobStoreService);
	}

	@Override
	protected LeaderElectionService createLeaderElectionService(String leaderPath) {
		log.debug("ZooKeeperHaServices createLeaderElectionService with leaderPath {},", leaderPath);
	...
	protected LeaderRetrievalService createLeaderRetrievalService(String leaderPath) {
		log.debug("ZooKeeperHaServices createLeaderRetrievalService with leaderPath {},", leaderPath);
	...
	protected String getLeaderPathForResourceManager() {
		log.debug("ZooKeeperHaServices return leaderPath for resource manager: {},", ZooKeeperUtils.getLeaderPathForResourceManager());
		return ZooKeeperUtils.getLeaderPathForResourceManager();
	}

	@Override
	protected String getLeaderPathForDispatcher() {
		log.debug("ZooKeeperHaServices return leaderPath for dispatcher: {},", ZooKeeperUtils.getLeaderPathForDispatcher());
		return ZooKeeperUtils.getLeaderPathForDispatcher();
	}

	@Override
	public String getLeaderPathForJobManager(final JobID jobID) {
		log.debug("ZooKeeperHaServices return leaderPath for job manager: {},", ZooKeeperUtils.getLeaderPathForJobManager(jobID));
		return ZooKeeperUtils.getLeaderPathForJobManager(jobID);
	}

	@Override
	protected String getLeaderPathForRestServer() {
		log.debug("ZooKeeperHaServices return leaderPath for rest server: {},", ZooKeeperUtils.getLeaderPathForRestServer());
