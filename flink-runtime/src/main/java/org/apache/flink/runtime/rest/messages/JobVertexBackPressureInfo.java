...
public class JobVertexBackPressureInfo implements ResponseBody {
	...
	public static final class SubtaskBackPressureInfo {
		...
		public boolean equals(Object o) {
			...
			final double dis = 1e-6;
			return subtask == that.subtask
				&& Math.abs(backPressuredRatio - that.backPressuredRatio) < dis
				&& Math.abs(idleRatio - that.idleRatio) < dis
				&& Math.abs(busyRatio - that.busyRatio) < dis
				&& Objects.equals(backpressureLevel, that.backpressureLevel);
		}

		@Override
		public int hashCode() {
			return Objects.hash(subtask, backpressureLevel, backPressuredRatio, idleRatio, busyRatio);
		}
