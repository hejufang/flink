...
public class HeapKeyedStateBackend<K> extends AbstractKeyedStateBackend<K> {
	...
	private <N, V> StateTable<K, N, V> tryRegisterStateTable(
		...
		if (stateTable != null) {
			...
			if (stateCompatibility.isCompatibleAfterMigration()) {
				...
				throw new StateMigrationException("For heap backends, the new state serializer must not be incompatible, details: " + stateCompatibility.getMessage());
			}
	...
	private <V, N> void migrateStateValues(StateDescriptor<?, V> stateDesc, RegisteredKeyValueStateBackendMetaInfo<N, V> oldStateInfo) throws Exception {
