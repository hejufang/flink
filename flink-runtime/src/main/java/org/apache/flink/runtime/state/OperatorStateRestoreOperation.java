...
public class OperatorStateRestoreOperation implements RestoreOperation<Void> {
    ...
    public Void restore() throws Exception {
        ...
        if (restoreThreads == 1) {
            ...
            try {
                // submit all tasks
                List<CompletableFuture<StateHandleRestoreState>> futures =
                        new ArrayList<>(stateHandles.size());
                for (OperatorStateHandle stateHandle : stateHandles) {
                    futures.add(
                            CompletableFuture.supplyAsync(
                                    new StateHandleParserSupplier(
                                            stateHandle,
                                            userClassloader,
                ...
                FutureUtils.combineAll(futures)
                        .thenAccept(
                                (Collection<StateHandleRestoreState> states) -> {
    ...
    /**
     * Most are copied from {@link #restoreInSingleThread()} instead of reorganizing the codes,
     * because the recovery process here is extremely important and we'd better not touch the
    ...
    private void restoreStatesInHandle(StateHandleRestoreState state) throws Exception {
        ...
        for (StateMetaInfoSnapshot restoredSnapshot :
                state.getRestoredOperatorMetaInfoSnapshots()) {
            ...
            if (restoredMetaInfo.getPartitionStateSerializer()
                ...
                // must fail now if the previous typeSerializer cannot be restored because there is
                // no typeSerializer
                // capable of reading previous state
                // TODO when eager state registration is in place, we can try to get a convert
                // deserializer
            ...
            if (null == listState) {
                ...
                // TODO with eager state registration in place, check here for typeSerializer
                // migration strategies
            }
        }

        for (StateMetaInfoSnapshot restoredSnapshot :
                state.getRestoredBroadcastMetaInfoSnapshots()) {
        ...
        for (Map.Entry<String, BackendWritableBroadcastState<?, ?>> entry :
                state.getBroadcastStates().entrySet()) {
            BackendWritableBroadcastState broadcastStateForName =
                    registeredBroadcastStates.get(entry.getKey());
        ...
        public StateHandleRestoreState get() {
            ...
            try {
                ...
                for (StateMetaInfoSnapshot restoredSnapshot : restoredOperatorMetaInfoSnapshots) {
                    ...
                    PartitionableListState<?> listState =
                            tempOperatorStates.get(restoredSnapshot.getName());
                ...
                for (StateMetaInfoSnapshot restoredSnapshot : restoredBroadcastMetaInfoSnapshots) {
                    ...
                    BackendWritableBroadcastState<?, ?> broadcastState =
                            tempBroadcastStates.get(restoredSnapshot.getName());

                    if (broadcastState == null) {
                        broadcastState = new HeapBroadcastState<>(restoredMetaInfo);

                        tempBroadcastStates.put(
                                broadcastState.getStateMetaInfo().getName(), broadcastState);
                    } else {
                        // TODO with eager state registration in place, check here for
                        // typeSerializer migration strategies
                ...
                for (Map.Entry<String, OperatorStateHandle.StateMetaInfo> nameToOffsets :
                    ...
                    if (listStateForName == null) {
                        BackendWritableBroadcastState<?, ?> broadcastStateForName =
                                tempBroadcastStates.get(stateName);
                        Preconditions.checkState(
                                broadcastStateForName != null,
                                "Found state without " + "corresponding meta info: " + stateName);
                        deserializeBroadcastStateValues(
                ...
                return new StateHandleRestoreState(
                        tempOperatorStates,
                        tempBroadcastStates,
