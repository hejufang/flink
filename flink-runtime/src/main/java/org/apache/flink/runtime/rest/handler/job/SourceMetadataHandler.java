...
/**
 * Request handler for source meta used in TSS.
 */
	...
	protected CompletableFuture<JobSourceMetaMetricsInfo> handleRequest(
		...
		if (fetcher != null) {
			...
			for (Map.Entry<String, MetricStore.JobMetricStore> jobMetricStoreEntry : jobMetricStoreMap.entrySet()) {
				...
				for (Map.Entry<String, MetricStore.TaskMetricStore> taskStore : taskMetricsStores.entrySet()) {
					list.add(SourceMetaMetrics.parseConsumerMetaInfo(taskStore.getValue().metrics));
				}
			}
