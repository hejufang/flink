...
public class SavepointDumpHandler
		extends AbstractRestHandler<RestfulGateway, EmptyRequestBody, SavepointDumpInfo, SavepointDumpMessageParameters> {

	public SavepointDumpHandler(
			GatewayRetriever<? extends RestfulGateway> leaderRetriever,
			Time timeout,
			Map<String, String> responseHeaders,
			MessageHeaders<EmptyRequestBody, SavepointDumpInfo, SavepointDumpMessageParameters> messageHeaders) {
		super(leaderRetriever, timeout, responseHeaders, messageHeaders);
	}

	@Override
	protected CompletableFuture<SavepointDumpInfo> handleRequest(
			@Nonnull HandlerRequest<EmptyRequestBody, SavepointDumpMessageParameters> request,
			@Nonnull RestfulGateway gateway) throws RestHandlerException {
		List<String> pendingSavepointIds = new ArrayList<>();
		final JobID jobID = request.getPathParameter(JobIDPathParameter.class);
		try {
			pendingSavepointIds.addAll(gateway.dumpPendingSavepoints(jobID).get(timeout.toMilliseconds(), TimeUnit.MILLISECONDS));
		} catch (Exception e) {
