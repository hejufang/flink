...
public class PreviewDataHandler
        extends AbstractRestHandler<
                RestfulGateway, EmptyRequestBody, PreviewDataResponse, PreviewDataParameters> {
    ...
    public PreviewDataHandler(
            ...
            MessageHeaders<EmptyRequestBody, PreviewDataResponse, PreviewDataParameters>
                    messageHeaders,
    ...
    @Override
    public CompletableFuture<PreviewDataResponse> handleRequest(
        ...
        if (jobIDParameters.isEmpty()) {
            return FutureUtils.completedExceptionally(
                    new IllegalArgumentException("jobId must be provided"));
        }
        JobID jobId = jobIDParameters.get(0);

        List<JobVertexID> jobVertexIDParameters =
                request.getQueryParameter(PreviewDataJobVertexIdParameter.class);
        if (jobVertexIDParameters.isEmpty()) {
            return FutureUtils.completedExceptionally(
                    new IllegalArgumentException("vertexId must be provided"));
        }
            ...
            final ResourceManagerGateway resourceManagerGateway =
                    resourceManagerGatewayRetriever
                            ...
                            .orElseThrow(
                                    () -> {
                                        log.debug(
                                                "Could not connect to ResourceManager right now.");
                                        return new RestHandlerException(
                                                "Cannot connect to ResourceManager right now. Please try to refresh.",
                                                HttpResponseStatus.NOT_FOUND);
                                    });
            return resourceManagerGateway.requestTaskManagerPreviewData(
                    taskManagerId, jobId, jobVertexId, timeout);
