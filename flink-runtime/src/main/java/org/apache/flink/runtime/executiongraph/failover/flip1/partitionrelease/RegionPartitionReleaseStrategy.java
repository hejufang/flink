...
public class RegionPartitionReleaseStrategy implements PartitionReleaseStrategy {
	...
	private final Map<ConsumedPartitionGroup, Set<SchedulingPipelinedRegion>> partitionGroupConsumerRegions = new HashMap<>();

	/**
	 * PartitionID -> PartitionID group.
	 */
	private final Map<IntermediateResultPartitionID, List<ConsumedPartitionGroup>> partitionGroupsById = new HashMap<>();

	/**
	 * Region -> Region set. Only contains regions with consumed results.
	 */
	private final Map<SchedulingPipelinedRegion, List<Set<SchedulingPipelinedRegion>>> consumerRegionSetMapping = new HashMap<>();

	/**
	 * The progress of Region set. Only contains regions with consumed results.
	 */
	private final Map<Set<SchedulingPipelinedRegion>, Set<SchedulingPipelinedRegion>> consumerRegionFinishProgress = new HashMap<>();

	/**
	 * The progress of PartitionID group.
	 */
	private final Map<ConsumedPartitionGroup, Boolean> partitionGroupFinishProgress = new HashMap<>();
	...
	private void init() {
		...
		initPartitionGroupConsumerRegions(schedulingTopology.getAllPipelinedRegions(), partitionGroupConsumerRegions);
		...
		for (Set<SchedulingPipelinedRegion> schedulingPipelinedRegionSet : partitionGroupConsumerRegions.values()) {
			...
			consumerRegionFinishProgress.computeIfAbsent(schedulingPipelinedRegionSet, set -> new HashSet<>());
		}
	...
	public List<IntermediateResultPartitionID> vertexFinished(final ExecutionVertexID finishedVertex) {
		...
		if (regionExecutionView.isFinished()) {
			...
			for (Set<SchedulingPipelinedRegion> regionSet : consumerRegionSetMapping.getOrDefault(pipelinedRegion, Collections.emptyList())) {
	...
	private List<IntermediateResultPartitionID> filterReleasablePartitions(final List<ConsumedPartitionGroup> schedulingResultPartitions) {
		...
		for (ConsumedPartitionGroup group : schedulingResultPartitions) {
			final Set<SchedulingPipelinedRegion> consumerRegionSet = partitionGroupConsumerRegions.get(group);
			if (consumerRegionFinishProgress.get(consumerRegionSet).size() == consumerRegionSet.size()) {
				partitionGroupFinishProgress.replace(group, Boolean.TRUE);
				for (IntermediateResultPartitionID partitionId : group.getResultPartitions()) {
					if (partitionGroupsById.get(partitionId).stream().map(partitionGroupFinishProgress::get).allMatch(progress -> progress == Boolean.TRUE)) {
