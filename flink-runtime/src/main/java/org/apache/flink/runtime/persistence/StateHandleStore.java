 ...
 *
 * <p>To avoid concurrent modification issues, the implementation needs to ensure that only the leader could
 * update the state store.
 *
 * <p>Even some methods name contains the "lock"(e.g. {@link #getAndLock}), it does not mean the implementation has to
 * actually lock a specific state handle. Also it could have an empty implementation for release operation.
 *
...
public interface StateHandleStore<T extends Serializable, R extends ResourceVersion<R>> {
	...
	/**
	 * Persist the state to distributed storage(e.g. S3, HDFS, etc.). And then creates a state handle, stores it in
	 * the distributed coordination system(e.g. ZooKeeper, Kubernetes, etc.).
	 *
	 ...
	 *
	 * @param name Key name in ConfigMap or child path name in ZooKeeper
	 * @param resourceVersion resource version of previous storage object. If the resource version does not match, the
	 * replace operation will fail. Since there is an unexpected update operation snuck in.
	 ...
	 *
	 * @return List of valid state handle name. The name is key name in ConfigMap or child path name in ZooKeeper.
	 *
	 ...
	 * It returns the {@link RetrievableStateHandle} stored under the given state node if any. Also the state on
	 * the external storage will be discarded.
	 *
	 * @param name Key name in ConfigMap or child path name in ZooKeeper
	 *
	...
	/**
	 * Releases the lock on the specific state handle so that it could be deleted by other {@link StateHandleStore}.
	 * If no lock exists or the underlying storage does not support, nothing will happen.
	 *
	 ...
	 * {@link StateHandleStore}. If no lock exists or the underlying storage does not support, nothing will happen.
	 *
