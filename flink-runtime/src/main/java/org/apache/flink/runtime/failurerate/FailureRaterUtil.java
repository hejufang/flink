...
public class FailureRaterUtil {
	...
	public static FailureRater createFailureRater(
		...
		int rate = configuration.getInteger(ResourceManagerOptions.MAXIMUM_WORKERS_FAILURE_RATE);
		...
		if (rate >= 0 && ratio < 0) {
			return new TimestampBasedFailureRater(rate, Time.of(failureIntervalMs, TimeUnit.MILLISECONDS));
		}
