...
import org.apache.flink.runtime.rest.handler.smartresource.SmartResourceHandler;
...
public class WebMonitorEndpoint<T extends RestfulGateway> extends RestServerEndpoint implements LeaderContender, JsonArchivist {
	...
	protected List<Tuple2<RestHandlerSpecification, ChannelInboundHandler>> initializeHandlers(final CompletableFuture<String> localAddressFuture) {
		...
		SmartResourceHandler smartResourceHandler = new SmartResourceHandler(
			...
			timeout,
			responseHeaders,
			SmartResourceHeaders.getInstance()
		);
		...
		final TaskManagerThreadDumpHandler taskManagerThreadDumpFileHandler = new TaskManagerThreadDumpHandler(
			leaderRetriever,
			timeout,
			responseHeaders,
		...
		final PreviewDataHandler previewDataHandler = new PreviewDataHandler(
			leaderRetriever,
			timeout,
			responseHeaders,
	...
	public void grantLeadership(final UUID leaderSessionID) {
		log.info("{} was granted leadership with leaderSessionID={}", getLeaderAddress(), leaderSessionID);
		leaderElectionService.confirmLeadership(leaderSessionID, getLeaderAddress());
	}

	@Override
	public void revokeLeadership() {
		log.info("{} lost leadership", getLeaderAddress());
	}
