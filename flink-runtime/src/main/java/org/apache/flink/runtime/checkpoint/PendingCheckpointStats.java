...
public class PendingCheckpointStats extends AbstractCheckpointStats {
	...
	private void removeSubtaskStats(JobVertexID jobVertexId, SubtaskStateStats subtask) {
		...
		if (taskStateStats != null ) {
			...
			if (oldSubtask != null && taskStateStats.removeSubtaskStats(oldSubtask.getSubtaskIndex())) {
				...
				long alignmentBuffered = oldSubtask.getAlignmentBuffered();
	...
	CompletedCheckpointStats.DiscardCallback reportCompletedCheckpoint(String externalPointer, long finishTimestamp, long totalStateSize) {
		return reportCompletedCheckpoint(externalPointer, finishTimestamp, totalStateSize, numberOfSubtasks);
	}

	CompletedCheckpointStats.DiscardCallback reportCompletedCheckpoint(String externalPointer, long finishTimestamp, long totalStateSize, int numNeedAcknowledgedSubtasks) {
	...
	void reportFailedCheckpoint(long failureTimestamp, @Nullable Throwable cause) {
		reportFailedCheckpoint(failureTimestamp, cause, null);
	}
