...
/**
 * Preview connector used to preview sink data for validate SQL logic, now used for sql debug internal.
 */
public class PreviewTableSinkFactory implements DynamicTableSinkFactory {
	...
	public static final ConfigOption<Boolean> CHANGELOG_MODE_ENABLE = ConfigOptions
		.key("changelog-mode.enable")
		.booleanType()
		.defaultValue(true)
		.withDescription("Enable changelog mode, can be controlled to generate and obtain this mode data.");

	public static final ConfigOption<Integer> CHANGE_RESULT_ROWS_MAX = ConfigOptions
		.key("changelog-mode.result-rows.max")
		.intType()
		.defaultValue(10000)
		.withDescription("The number of rows to cache 	when in the changelog result. " +
			"If the number of rows exceeds the specified value, it retries the row in the FIFO style.");

	public static final ConfigOption<Boolean> TABLE_MODE_ENABLE = ConfigOptions
		.key("table-mode.enable")
		.booleanType()
		.defaultValue(true)
		.withDescription("Enable table mode, can be controlled to generate and obtain this mode data.");

	public static final ConfigOption<Integer> TABLE_RESULT_ROWS_MAX = ConfigOptions
		.key("table-mode.result-rows.max")
		.intType()
		.defaultValue(10000)
		.withDescription("The number of rows to cache when in the table result. " +
			"If the number of rows exceeds the specified value, it retries the row in the FIFO style.");

	public static final ConfigOption<Boolean> INTERNAL_TEST_ENABLE = ConfigOptions
		.key("internal-test.enable")
		.booleanType()
		.defaultValue(false)
		.withDescription("If internal test enable, the result will send to system out stream for test.");
