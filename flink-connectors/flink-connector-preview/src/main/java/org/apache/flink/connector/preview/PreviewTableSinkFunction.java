...
public class PreviewTableSinkFunction extends PreviewSinkFunction<RowData> implements SpecificParallelism {
	...
	private void initResultSizeSpace(MemorySize akkaFrameSize){
		...
		changeLogResult = new ResultDataManager<>(previewTableOptions.getChangelogRowsMax(), maxResultByteSize, null);
		tableResult = new ResultDataManager<>(previewTableOptions.getTableRowsMax(), maxResultByteSize,
			serializationSchema);
	}
	...
	public PreviewDataResponse getPreviewData(PreviewDataRequest previewDataRequest) {
		synchronized (this) {
			...
			LOG.info("previewFunction return data, changeLogResult size: {}, tableResult size: {}",
				changeLogResult.getResultList().size(), tableResult.getResultList().size());
	...
	private void processChangeLogResult(RowData rowData) {
		final String data = rowData.getRowKind().shortString() + new String(serializationSchema.serialize(rowData));
	...
	private static class ResultDataManager<T>{
		...
		public ResultDataManager(int maxResultCount, long maxResultByteSize,
		...
		private boolean applyResultSizeSpace(int byteSize) {
			...
			if (byteSize >= this.maxResultByteSize){
				final String msg = String.format("The size of a single record exceeds the maximum limit. "
					+ "Record size: %s, maxResultSize: %s", byteSize, this.maxResultByteSize);
				throw new FlinkRuntimeException(msg);
			}
			if (this.currentResultByteSize + byteSize >= this.maxResultByteSize) {
				LOG.debug("space is full. totalBytes: {}, content length: {}", this.currentResultByteSize, byteSize);
				return false;
			}
