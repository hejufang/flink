...
public class PreviewTableSinkITTest {
	...
	public void testPreviewSinkChangeLogOnly() throws Exception {
		...
		String sinkDDL = " CREATE  TABLE preview_test (name VARCHAR, cnt INT)\n" +
			"    WITH (\n" +
			"    'connector' = 'preview', \n" +
		...
		String query = "INSERT INTO preview_test SELECT word, sum(frequency) FROM src group by word";
	...
	public void testPreviewSinkRowMax() throws Exception {
		...
		TableResult tableResult = tEnv.executeSql(query);
		tableResult.getJobClient().get().getJobExecutionResult(Thread.currentThread().getContextClassLoader()).get();
		assertEquals(ResourceUtil.readFileContent(PATH + "changelog_table_row_max.out"), out.toString());
	}
}
