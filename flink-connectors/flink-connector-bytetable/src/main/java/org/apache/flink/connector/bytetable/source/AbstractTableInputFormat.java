...
/**
 * Abstract {@link InputFormat} to read data from ByteTable tables.
 */
@Internal
abstract class AbstractTableInputFormat<T> extends RichInputFormat<T, TableInputSplit> {
	...
	public AbstractTableInputFormat(org.apache.hadoop.conf.Configuration hConf) {
		serializedConfig = ByteTableConfigurationUtil.serializeConfiguration(hConf);
	}
	...
	/**
	 * ByteTable returns an instance of {@link Result}.
	 *
	 * <p>This method maps the returned {@link Result} instance into the output type {@link T}.
	 *
	 * @param r The Result instance from ByteTable that needs to be converted
	...
	protected org.apache.hadoop.conf.Configuration getHadoopConfiguration() {
		return ByteTableConfigurationUtil.deserializeConfiguration(serializedConfig, HBaseConfiguration.create());
	}
	...
	public T nextRecord(T reuse) throws IOException {
		...
		try {
			...
			//current hbase dependecy version can not use withStartRow().
			scan.setStartRow(currentRow);
			resultScanner = table.getScanner(scan);
			res = resultScanner.next();
		}
