...
/**
 * A utility class to process data exchange with Bytetable type system.
 */
@Internal
public class ByteTableTypeUtils {
	...
	/**
	 * Gets the type index (type representation in ByteTable connector) from the {@link TypeInformation}.
	 */
	...
	/**
	 * Checks whether the given Class is a supported type in ByteTable connector.
	 */
	...
	/**
	 * Checks whether the given {@link LogicalType} is supported in ByteTable connector.
	 */
				...
				if (timePrecision < MIN_TIME_PRECISION || timePrecision > MAX_TIME_PRECISION) {
					throw new UnsupportedOperationException(
						String.format("The precision %s of TIME type is out of the range [%s, %s] supported by " +
							"ByteTable connector", timePrecision, MIN_TIME_PRECISION, MAX_TIME_PRECISION));
				}
				return true;
				...
				if (timestampPrecision < MIN_TIMESTAMP_PRECISION || timestampPrecision > MAX_TIMESTAMP_PRECISION) {
					throw new UnsupportedOperationException(
						String.format("The precision %s of TIMESTAMP type is out of the range [%s, %s] supported by " +
							"ByteTable connector", timestampPrecision, MIN_TIMESTAMP_PRECISION, MAX_TIMESTAMP_PRECISION));
				}
				return true;
