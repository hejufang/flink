...
import org.apache.hudi.sink.BucketStreamWriteOperatorFactory;
...
import org.apache.hudi.sink.partitioner.HiveBucketPartitioner;
 ...
 * Hoodie table sink.
 *
 * Copied from Hudi, will be removed once HUDI-2142 is fixed.
 */
public class HoodieTableSink implements DynamicTableSink, SupportsPartitioning, SupportsOverwrite {
  ...
  public SinkRuntimeProvider getSinkRuntimeProvider(Context context) {
    return (DataStreamSinkProvider) dataStream -> {
      ...
      if (StreamerUtil.useBucketIndex(conf)) {
        BucketStreamWriteOperatorFactory<HoodieRecord> operatorFactory = new BucketStreamWriteOperatorFactory<>(conf);
        int bucketNum = conf.getInteger(FlinkOptions.WRITE_HIVE_BUCKET_NUM);
        Partitioner bucketPartitioner = new HiveBucketPartitioner(bucketNum);
        pipeline = hoodieDataStream
            .partitionCustom(bucketPartitioner, HoodieRecord::getRecordKey)
