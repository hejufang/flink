...
public class DDLOperationConverter {
	...
	private Operation convertCreateAlterView(HiveParserCreateViewDesc desc) {
		...
		TableSchema schema = HiveTableUtil.createTableSchema(desc.getSchema(), Collections.emptyList(),
		...
		CatalogView catalogView = new CatalogViewImpl(desc.getOriginalText(),
				desc.getExpandedText(), schema, props, comment);
		if (desc.isAlterViewAs()) {
	...
	private Operation convertDropPartitions(DropPartitionDesc desc) {
		ObjectIdentifier tableIdentifier = catalogManager.qualifyIdentifier(UnresolvedIdentifier.of(desc.getDbName(), desc.getTableName()));
		...
		List<CatalogPartitionSpec> specs = desc.getSpecs().stream().map(CatalogPartitionSpec::new).collect(Collectors.toList());
	...
	private Operation convertAddPartitions(AddPartitionDesc desc) {
		ObjectIdentifier tableIdentifier = desc.getDbName() == null ? parseObjectIdentifier(desc.getTableName()) :
				catalogManager.qualifyIdentifier(UnresolvedIdentifier.of(desc.getDbName(), desc.getTableName()));
		CatalogBaseTable catalogBaseTable = getCatalogBaseTable(tableIdentifier);
		if (catalogBaseTable instanceof CatalogView) {
			throw new ValidationException("ALTER TABLE for a view is not allowed");
		}
	...
	private Operation convertAlterView(HiveParserAlterTableDesc desc) {
		...
		switch (desc.getOp()) {
			case RENAME:
				return new AlterViewRenameOperation(viewIdentifier, parseObjectIdentifier(desc.getNewName()));
			case ADDPROPS:
				Map<String, String> props = new HashMap<>(oldView.getOptions());
				props.putAll(desc.getProps());
				CatalogView newView = new CatalogViewImpl(oldView.getOriginalQuery(), oldView.getExpandedQuery(),
						oldView.getSchema(), props, oldView.getComment());
	...
	private Operation convertAlterTable(HiveParserAlterTableDesc desc) {
		...
		CatalogPartition catalogPartition = partSpec != null ? getPartition(tableIdentifier, partSpec) : null;
		Map<String, String> newProps = new HashMap<>();
		switch (desc.getOp()) {
			case RENAME:
				return new AlterTableRenameOperation(tableIdentifier, parseObjectIdentifier(desc.getNewName()));
			case ADDPROPS:
				newProps.put(ALTER_TABLE_OP, CHANGE_TBL_PROPS.name());
				newProps.putAll(desc.getProps());
				return convertAlterTableProps(tableIdentifier, oldTable, partSpec, catalogPartition, newProps);
			case ALTERLOCATION:
			...
			case ADDSERDEPROPS:
				...
				return convertAlterTableProps(tableIdentifier, oldTable, partSpec, catalogPartition, newProps);
			case REPLACECOLS:
				return convertAddReplaceColumns(tableIdentifier, oldTable, desc.getNewCols(), true, desc.isCascade());
			case ADDCOLS:
				return convertAddReplaceColumns(tableIdentifier, oldTable, desc.getNewCols(), false, desc.isCascade());
			case RENAMECOLUMN:
				return convertChangeColumn(tableIdentifier, oldTable, desc.getOldColName(), desc.getNewColName(),
						desc.getNewColType(), desc.getNewColComment(), desc.getAfter(), desc.isFirst(), desc.isCascade());
			default:
				throw new FlinkHiveException("Unsupported alter table operation " + desc.getOp());
		}
	}

	private Operation convertChangeColumn(ObjectIdentifier tableIdentifier, CatalogTable oldTable, String oldName,
			String newName, String newType, String newComment, String after, boolean first, boolean cascade) {
		...
		TableColumn newTableColumn = TableColumn.of(newName, HiveTypeUtil.toFlinkType(TypeInfoUtils.getTypeInfoFromTypeString(newType)));
	...
	private Operation convertAddReplaceColumns(ObjectIdentifier tableIdentifier, CatalogTable oldTable, List<FieldSchema> newCols,
			boolean replace, boolean cascade) {
		// prepare properties
		Map<String, String> props = new HashMap<>(oldTable.getOptions());
		props.put(ALTER_TABLE_OP, ALTER_COLUMNS.name());
		if (cascade) {
		...
		for (FieldSchema col : newCols) {
			builder.add(TableColumn.of(
					col.getName(), HiveTypeUtil.toFlinkType(TypeInfoUtils.getTypeInfoFromTypeString(col.getType()))));
		}
		// add part cols
	...
	private Operation convertAlterTableProps(ObjectIdentifier tableIdentifier, CatalogTable oldTable,
			CatalogPartitionSpec partSpec, CatalogPartition catalogPartition, Map<String, String> newProps) {
		Map<String, String> props = new HashMap<>();
		if (catalogPartition != null) {
			props.putAll(catalogPartition.getProperties());
			props.putAll(newProps);
			return new AlterPartitionPropertiesOperation(tableIdentifier, partSpec, new CatalogPartitionImpl(props, catalogPartition.getComment()));
		} else {
			props.putAll(oldTable.getOptions());
			props.putAll(newProps);
			return new AlterTablePropertiesOperation(tableIdentifier, oldTable.copy(props));
		}
	}
	...
	private void markNonGeneric(Map<String, String> props) {
		props.put(CatalogConfig.IS_GENERIC, "false");
	}
	...
	private CatalogBaseTable getCatalogBaseTable(ObjectIdentifier tableIdentifier, boolean ifExists) {
		...
		if (!optionalCatalogTable.isPresent()) {
			if (ifExists) {
				...
				throw new ValidationException(String.format("Table or View %s doesn't exist.",
						tableIdentifier.toString()));
			}
		}
		if (optionalCatalogTable.get().isTemporary()) {
			throw new ValidationException(String.format("Table or View %s is temporary.",
					tableIdentifier.toString()));
		}
	...
	private CatalogPartition getPartition(ObjectIdentifier tableIdentifier, CatalogPartitionSpec partitionSpec) {
		return catalogManager.getPartition(tableIdentifier, partitionSpec)
	...
	private ObjectIdentifier parseObjectIdentifier(String compoundName) {
		UnresolvedIdentifier unresolvedIdentifier = UnresolvedIdentifier.of(compoundName.split("\\."));
