...
public class HivePermissionUtils {
	...
	private static final String AUTHENTICATION_SERVER_PSM = "data.olap.gemini_readonly.service.lf.byted.org";
	private static final String AUTHENTICATION_SERVER_URL_TEMPLATE = "http://%s/api/query/verifyUsersPrivilege";
	...
	public static boolean checkPermission(
		...
		try {
			...
			if (hasPermission) {
				LOG.info("User: '{}' or psm: '{}' has {} permission for database: '{}', " +
					"table: '{}', columns: '{}'.", user, psm, permissionType, database, table, columns);
			} else {
				String errorMsg = String.format("Neither user: '%s' nor psm: '%s' " +
	...
	private static String getRequestUrl() {
		...
		Preconditions.checkArgument(serviceNodes != null && !serviceNodes.isEmpty(),
			String.format("serviceNodes of psm: '%s' is null or empty.", AUTHENTICATION_SERVER_PSM));
		Collections.shuffle(serviceNodes);
		ServiceNode serviceNode = serviceNodes.get(ThreadLocalRandom.current().nextInt(serviceNodes.size()));
	...
	private static String buildPostData(
		...
		try {
			...
			throw new FlinkRuntimeException("Fail to parse request to json string, request: " + request, e);
		}
	}
