...
import org.apache.flink.connectors.hive.udf.UDFExampleAdd;
...
public class HiveCompatibleITCase {
	...
	private static final String[] QUERIES = new String[]{
		...
		"select udf_add(i, 100) from bar",
		"select case when d > 1 then array('a') else sa end from bas",
		"select * from bar",
	...
	public void testHiveCompatible() throws Exception {
		...
		tableEnv.executeSql("create table bas(sa array<string>, d double)");
		...
		hiveShell.insertInto("default", "bas").addRow(Arrays.asList("a", "b", "c"), 3.0).commit();
		...
		tableEnv.executeSql(
			"create temporary function udf_add as '" + UDFExampleAdd.class.getName() +
				"' using jar 'hdfs://haruna/dp/dorado/udf/100000014/100000014.jar'");
		...
		// test set config
		tableEnv.executeSql("set hive.mapred.mode=nonstrict;");
