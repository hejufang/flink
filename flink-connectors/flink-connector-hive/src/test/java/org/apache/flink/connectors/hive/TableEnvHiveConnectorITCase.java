...
public class TableEnvHiveConnectorITCase {
	...
	public void testDecimal() throws Exception {
		...
		try {
			...
			TableEnvUtil.execInsertSqlAndWaitResult(tableEnv, "insert into db1.src2 values (1.0),(2.12),(5.123),(5.456),(123456789.12)");
	...
	public void testPartialDynamicPartition() throws Exception {
		...
		try {
			...
			TableEnvUtil.execInsertSqlAndWaitResult(tableEnv, "insert into db1.dest partition (p1=1.1,p2) select x,y from db1.src");
	...
	public void testViews() throws Exception {
		...
		try {
			...
			results = Lists.newArrayList(tableEnv.sqlQuery("select * from db1.v2 order by key").execute().collect());
			assertEquals("[1,3, 3,2]", results.toString());
			results = Lists.newArrayList(tableEnv.sqlQuery("select * from db1.v3 order by key").execute().collect());
	...
	public void testParquetNameMapping() throws Exception {
		...
		try {
			...
			assertEquals("[1, 2]", Lists.newArrayList(tableEnv.sqlQuery("select x from db1.t1 order by x").execute().collect()).toString());
			assertEquals("[1, 2]", Lists.newArrayList(tableEnv.sqlQuery("select x from db1.t2 order by x").execute().collect()).toString());
		} finally {
			tableEnv.executeSql("drop database db1 cascade");
		}
	}
	...
	public void testInsertPartitionWithStarSource() throws Exception {
		...
		TableEnvUtil.execInsertSqlAndWaitResult(tableEnv,
				"insert into dest partition (p1=1,p2) select * from src");
	...
	public void testInsertPartitionWithValuesSource() {
		...
		tableEnv.executeSql("create table dest (x int) partitioned by (p1 int,p2 string)");
		TableEnvUtil.execInsertSqlAndWaitResult(tableEnv,
				"insert into dest partition (p1=1,p2) values(1, 'a')");
		List<Row> results = Lists.newArrayList(tableEnv.sqlQuery("select * from dest").execute().collect());
		assertEquals("[1,1,a]", results.toString());
		tableEnv.executeSql("drop table if exists dest");
	}
