...
import org.apache.flink.cep.MultiplePatternFlatTimeoutFunction;
 ...
 * @param <T>
 */
public class MultiplePatternTimeoutFlatSelectAdapter<IN, OUT, T>
		extends MultiplePatternFlatSelectAdapter<IN, OUT>
		implements MultiplePatternTimedOutPartialMatchHandler<IN> {

	private final MultiplePatternFlatTimeoutFunction<IN, T> flatTimeoutFunction;
	private final OutputTag<T> timedOutPartialMatchesTag;

	private transient SideCollector<T> sideCollector;

	public MultiplePatternTimeoutFlatSelectAdapter(
			MultiplePatternFlatSelectFunction<IN, OUT> flatSelectFunction,
			MultiplePatternFlatTimeoutFunction<IN, T> flatTimeoutFunction,
