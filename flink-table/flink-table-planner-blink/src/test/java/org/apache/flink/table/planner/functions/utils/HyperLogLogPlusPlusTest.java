...
public class HyperLogLogPlusPlusTest {
	...
	public void testAddNulls() {
		Tuple3<HyperLogLogPlusPlus, GenericRowData, HllAccumulator> tuple3 = createEstimator(0.05);
		HyperLogLogPlusPlus hll = tuple3.f0;
		GenericRowData input = tuple3.f1;
	...
	private void testCardinalityEstimates(
		...
		rsds.stream().flatMap(rsd -> {
			...
			int n = tuple2.f1;
			Tuple3<HyperLogLogPlusPlus, GenericRowData, HllAccumulator> tuple3 = createEstimator(rsd);
			HyperLogLogPlusPlus hll = tuple3.f0;
			GenericRowData input = tuple3.f1;
			HllAccumulator accumulator = tuple3.f2;

			for (int i = 0; i < n; ++i) {
				input.setField(0, f.apply(i));
				hll.update(accumulator, input.getField(0));
			}
	...
	private Tuple3<HyperLogLogPlusPlus, GenericRowData, HllAccumulator> createEstimator(double rsd) {
