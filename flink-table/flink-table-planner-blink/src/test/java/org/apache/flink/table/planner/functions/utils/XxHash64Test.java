...
/**
 * Test the {@link XxHash64} function.
 *
...
public class XxHash64Test {
	...
	public void testKnownIntegerInputs() {
		Assert.assertEquals(0x9256E58AA397AEF1L, XxHash64.hashInt(TEST_INT, DEFAULT_SEED));
		Assert.assertEquals(0x9D5FFDFB928AB4BL, XxHash64.hashInt(TEST_INT, PRIME));
	}

	@Test
	public void testKnownLongInputs() {
		Assert.assertEquals(0xF74CB1451B32B8CFL, XxHash64.hashLong(TEST_LONG, DEFAULT_SEED));
		Assert.assertEquals(0x9C44B77FBCC302C5L, XxHash64.hashLong(TEST_LONG, PRIME));
	}
	...
	public void testKnownByteArrayInputs() {
		...
		Assert.assertEquals(0xEF46DB3751D8E999L,
				XxHash64.hashUnsafeBytes(hms, offset, 0, DEFAULT_SEED));
		Assert.assertEquals(0xAC75FDA2929B17EFL,
				XxHash64.hashUnsafeBytes(hms, offset, 0, PRIME));
		Assert.assertEquals(0x4FCE394CC88952D8L,
				XxHash64.hashUnsafeBytes(hms, offset, 1, DEFAULT_SEED));
		Assert.assertEquals(0x739840CB819FA723L,
				XxHash64.hashUnsafeBytes(hms, offset, 1, PRIME));

		if (ByteOrder.nativeOrder() == ByteOrder.LITTLE_ENDIAN) {
			Assert.assertEquals(0x9256E58AA397AEF1L,
					XxHash64.hashUnsafeBytes(hms, offset, 4, DEFAULT_SEED));
			Assert.assertEquals(0x9D5FFDFB928AB4BL,
					XxHash64.hashUnsafeBytes(hms, offset, 4, PRIME));
			Assert.assertEquals(0xF74CB1451B32B8CFL,
					XxHash64.hashUnsafeBytes(hms, offset, 8, DEFAULT_SEED));
			Assert.assertEquals(0x9C44B77FBCC302C5L,
					XxHash64.hashUnsafeBytes(hms, offset, 8, PRIME));
			Assert.assertEquals(0xCFFA8DB881BC3A3DL,
					XxHash64.hashUnsafeBytes(hms, offset, 14, DEFAULT_SEED));
			Assert.assertEquals(0x5B9611585EFCC9CBL,
					XxHash64.hashUnsafeBytes(hms, offset, 14, PRIME));
			Assert.assertEquals(0x0EAB543384F878ADL,
					XxHash64.hashUnsafeBytes(hms, offset, SIZE, DEFAULT_SEED));
			Assert.assertEquals(0xCAA65939306F1E21L,
					XxHash64.hashUnsafeBytes(hms, offset, SIZE, PRIME));
		} else {
	...
	public void randomizedStressTest() {
		...
		for (int i = 0; i < size; i++) {
			...
			Assert.assertEquals(XxHash64.hashInt(vint, DEFAULT_SEED),
					XxHash64.hashInt(vint, DEFAULT_SEED));
			Assert.assertEquals(XxHash64.hashLong(lint, DEFAULT_SEED),
					XxHash64.hashLong(lint, DEFAULT_SEED));

			hashcodes.add(XxHash64.hashLong(lint, DEFAULT_SEED));
		}
	...
	public void randomizedStressTestBytes() {
		...
		for (int i = 0; i < size; i++) {
			...
			Assert.assertEquals(
					XxHash64.hashUnsafeBytes(hms, 0, byteArrSize, DEFAULT_SEED),
					XxHash64.hashUnsafeBytes(hms, 0, byteArrSize, DEFAULT_SEED));

			hashcodes.add(XxHash64.hashUnsafeBytes(hms, 0, byteArrSize, DEFAULT_SEED));
		}
