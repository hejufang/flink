...
class FlinkRelMdUpsertKeysTest extends FlinkRelMdHandlerTestBase {
  ...
  def testGetUpsertKeysOnTableScan(): Unit = {
    ...
    val tableSourceScan = new StreamExecTableSourceScan(
      ...
      table)
    assertEquals(toBitSet(Array(0, 3)), mq.getUpsertKeys(tableSourceScan).toSet)
  }

  @Ignore("no needdd for now.")
  @Test
  ...
  def testGetUpsertKeysOnExpand(): Unit = {
    ...
    val outputRowType = ExpandUtil.buildExpandRowType(
    ...
    val expand = new LogicalExpand(cluster, studentLogicalScan.getTraitSet,
      studentLogicalScan, outputRowType, expandProjects, 7)
  ...
  def testGetUpsertKeysOnStreamExecDeduplicate(): Unit = {
    assertEquals(
      toBitSet(Array(1)),
      mq.getUpsertKeys(streamDeduplicateFirstRow).toSet)
    assertEquals(
      toBitSet(Array(1, 2)),
      mq.getUpsertKeys(streamDeduplicateLastRow).toSet)
  }
