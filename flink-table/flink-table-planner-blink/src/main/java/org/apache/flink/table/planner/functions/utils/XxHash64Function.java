...
/**
 * Compute hash value based on {@link XxHash64}.
 */
public class XxHash64Function {
	...
	public static long hash(Object value, long seed) {
		...
		if (value instanceof Boolean) {
			...
			if (!DecimalDataUtils.isByteArrayDecimal(bd.precision())) {
		...
		} else if (value instanceof BinaryStringData) {
			BinaryStringData bs = (BinaryStringData) value;
			MemorySegment[] segments = bs.getSegments();
			if (segments.length == 1) {
				return hashUnsafeBytes(segments[0], bs.getOffset(), bs.getSizeInBytes(), seed);
			} else {
				return hashUnsafeBytes(MemorySegmentFactory.wrap(bs.toBytes()), 0,
						bs.getSizeInBytes(), seed);
			}
		} else {
