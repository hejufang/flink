...
import org.apache.flink.table.planner.functions.utils.HyperLogLogPlusPlus;
import org.apache.flink.table.planner.functions.utils.XxHash64Function;
...
public abstract class ApproximateCountDistinctAggFunction<T>
	...
	public static class DecimalApproximateCountDistinctAggFunction
			extends ApproximateCountDistinctAggFunction<DecimalData> {

		private DecimalDataTypeInfo decimalTypeInfo;

		public DecimalApproximateCountDistinctAggFunction(DecimalDataTypeInfo decimalTypeInfo) {
			this.decimalTypeInfo = decimalTypeInfo;
		}

		public void accumulate(HllAccumulator accumulator, DecimalData input) throws Exception {
			super.accumulate(accumulator, input);
		}

		@Override
		long getHashcode(DecimalData d) {
			if (!DecimalDataUtils.isByteArrayDecimal(decimalTypeInfo.precision())) {
				...
				return XxHash64Function.hashUnsafeBytes(segment, 0, segment.size(),
						XxHash64Function.DEFAULT_SEED);
			}
		}
	}
	...
	public static class DoubleApproximateCountDistinctAggFunction
		...
		long getHashcode(Double value) {
			return XxHash64Function.hashLong(Double.doubleToLongBits(value),
					XxHash64Function.DEFAULT_SEED);
		}
	}
	...
	public static class FloatApproximateCountDistinctAggFunction
		...
		long getHashcode(Float value) {
			return XxHash64Function.hashInt(Float.floatToIntBits(value),
					XxHash64Function.DEFAULT_SEED);
		}
	}
	...
	public static class StringApproximateCountDistinctAggFunction
			extends ApproximateCountDistinctAggFunction<StringData> {

		public void accumulate(HllAccumulator accumulator, StringData input) throws Exception {
			if (input != null) {
				super.accumulate(accumulator, ((BinaryStringData) input).copy());
			}
		}

		@Override
		long getHashcode(StringData s) {
			BinaryStringData binaryStringData = (BinaryStringData) s;
			MemorySegment[] segments = binaryStringData.getSegments();
			if (segments.length == 1) {
				return XxHash64Function.hashUnsafeBytes(
						segments[0], binaryStringData.getOffset(), binaryStringData.getSizeInBytes(),
						XxHash64Function.DEFAULT_SEED);
			} else {
				return XxHash64Function.hashUnsafeBytes(
						MemorySegmentFactory.wrap(s.toBytes()), 0, binaryStringData.getSizeInBytes(),
						XxHash64Function.DEFAULT_SEED);
			}
		}
	}
