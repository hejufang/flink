...
object ScalarOperatorGens {
  ...
  /**
    ...
    */
  private def toPrimitive(
      ctx: CodeGeneratorContext,
      input: GeneratedExpression): GeneratedExpression = {
  ...
  private def generateArrayComparison(
    ...
    generateCallWithStmtIfArgsNotNull(ctx, new BooleanType(), Seq(left, right)) {
      args =>
        ...
        // E.g, the result of Array[1, null] = Array[1, null] is true for binary array,
        // and false for non-binary array.
        // todo: conform the behaviors in all cases.
        val stmt =
          s"""
             |boolean $resultTerm;
  ...
  private def generateRowComparison(
    ...
    generateCallWithStmtIfArgsNotNull(ctx, new BooleanType(), Seq(left, right)) {
      args =>
        ...
        val header =
          j"""
             |if ($leftTerm.getRowKind() != $rightTerm.getRowKind()) {
             |  $resultTerm = false;
             |}
          """.stripMargin
        ...
        val fieldCode = for (i <- fieldTypes.indices) yield {
          ...
          val leftReadCode = rowFieldReadAccess(ctx, i, leftTerm, fieldType)
          val rightReadCode = rowFieldReadAccess(ctx, i, rightTerm, fieldType)
