...
import org.apache.flink.table.planner.plan.nodes.physical.batch.{BatchExecGroupAggregateBase, BatchExecOverAggregate, BatchExecWindowAggregateBase}
import org.apache.flink.table.planner.plan.nodes.physical.stream.{StreamExecDeduplicate, StreamExecGlobalGroupAggregate, StreamExecGroupAggregate, StreamExecGroupWindowAggregate, StreamExecIntervalJoin, StreamExecLocalGroupAggregate, StreamExecOverAggregate}
...
class FlinkRelMdUpsertKeys private extends MetadataHandler[UpsertKeys] {
  ...
  def getUpsertKeys(rel: Sort, mq: RelMetadataQuery): JSet[ImmutableBitSet] = {
    ...
    filterKeys(
      FlinkRelMetadataQuery.reuseOrCreate(mq).getUpsertKeys(rel.getInput),
      ImmutableBitSet.of(rel.getCollation.getFieldCollations
        .map(f => f.getFieldIndex)
        .map(Integer.valueOf)
        .toList))
  }

  def getUpsertKeys(
      rel: StreamExecDeduplicate, mq: RelMetadataQuery): JSet[ImmutableBitSet] = {
  ...
  def getUpsertKeys(
      rel: BatchExecGroupAggregateBase, mq: RelMetadataQuery): JSet[ImmutableBitSet] = {
    if (rel.isFinal) {
  ...
  def getUpsertKeys(
      rel: StreamExecGroupAggregate, mq: RelMetadataQuery): JSet[ImmutableBitSet] = {
    FlinkRelMdUniqueKeys.INSTANCE.getUniqueKeysOnAggregate(rel.grouping)
  }

  def getUpsertKeys(
      rel: StreamExecLocalGroupAggregate, mq: RelMetadataQuery): JSet[ImmutableBitSet] = null

  def getUpsertKeys(
      rel: StreamExecGlobalGroupAggregate, mq: RelMetadataQuery): JSet[ImmutableBitSet] = {
    FlinkRelMdUniqueKeys.INSTANCE.getUniqueKeysOnAggregate(rel.grouping)
  }
  ...
  def getUpsertKeys(
      rel: BatchExecWindowAggregateBase, mq: RelMetadataQuery): util.Set[ImmutableBitSet] = {
    if (rel.isFinal) {
      FlinkRelMdUniqueKeys.INSTANCE.getUniqueKeysOnWindowAgg(
        rel.getRowType.getFieldCount,
        rel.namedProperties,
  ...
  def getUpsertKeys(
      rel: StreamExecGroupWindowAggregate, mq: RelMetadataQuery): util.Set[ImmutableBitSet] = {
    FlinkRelMdUniqueKeys.INSTANCE.getUniqueKeysOnWindowAgg(
      rel.getRowType.getFieldCount, rel.namedProperties, rel.grouping)
  }
  ...
  def getUpsertKeys(
      rel: BatchExecOverAggregate, mq: RelMetadataQuery): JSet[ImmutableBitSet] = {
    getUpsertKeysOnOver(rel, mq, ImmutableBitSet.of(rel.getGrouping: _*))
  }

  def getUpsertKeys(
      rel: StreamExecOverAggregate, mq: RelMetadataQuery): JSet[ImmutableBitSet] = {
  ...
  def getUpsertKeys(
      rel: StreamExecIntervalJoin, mq: RelMetadataQuery): JSet[ImmutableBitSet] = {
    val joinInfo = JoinInfo.of(rel.getLeft, rel.getRight, rel.joinCondition)
    getJoinUpsertKeys(joinInfo, rel.joinType, rel.getLeft, rel.getRight, mq)
  }

  def getUpsertKeys(
      join: CommonLookupJoin, mq: RelMetadataQuery): util.Set[ImmutableBitSet] = {
