...
import org.apache.flink.table.planner.plan.utils.{FlinkRelMdUtil, FlinkRelOptUtil, OperatorType}
...
trait BatchExecJoinRuleBase {
  ...
  protected def collectValidJoinHints(join: Join, tableConfig: TableConfig): JList[JoinStrategy] = {
    val allHints = join.getHints.asInstanceOf[JList[RelHint]]
  ...
  protected def checkBroadcast(
    ...
    if (withBroadcastHint) {
      ...
      val threshold = tableConfig.getConfiguration
