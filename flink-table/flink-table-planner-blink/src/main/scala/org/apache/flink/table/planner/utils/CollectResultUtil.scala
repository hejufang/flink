...
object CollectResultUtil {
  ...
  def collectSink[T](
    ...
    val fieldTypes = schema.getFieldDataTypes.map {
      t => TypeConversions.fromDataTypeToLegacyInfo(t)
    }
  ...
  private[flink] def collectConfiguredSink[T](
    ...
    val typeSerializer = TypeConversions
      .fromDataTypeToLegacyInfo(configuredSink.getConsumedDataType)
    ...
    val res = tEnv.execute(jobName.getOrElse(s"Flink collect table sink Job"))
