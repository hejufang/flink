...
public class TableSchema {
	...
	public String[] getPrunedColumnList(int[] fieldIndexes) {
		...
		for (int i = 0; i < fieldIndexes.length; i++) {
			...
			names[i] = getFieldName(index).orElseThrow(() -> new RuntimeException(
				String.format("column index %s out of range [0, %s].", index, columns.size() - 1)));
		}
		return names;
	}
	...
	public static class Builder {
		...
		/**
		 * Remove all physical columns and keep all computed columns.
		 */
		public Builder removePhysicalColumns() {
