...
public class SinkUpsertMaterializer extends TableStreamOperator<RowData>
	...
	public void open() throws Exception {
		...
		this.cache = new LRUMap<>(CACHE_SIZE, new LRUMap.RemovalListener<RowData, List<RowData>>() {
			@Override
			public void onRemoval(Map.Entry<RowData, List<RowData>> eldest) {
				RowData previousKey = (RowData) getCurrentKey();
				setCurrentKey(eldest.getKey());
				try {
					state.update(eldest.getValue());
				} catch (IOException e) {
	...
	public void processElement(StreamRecord<RowData> element) throws Exception {
		...
		List<RowData> values = cache.get(key);
		if (values == null) {
			values = state.value();
		}
		if (values == null) {
