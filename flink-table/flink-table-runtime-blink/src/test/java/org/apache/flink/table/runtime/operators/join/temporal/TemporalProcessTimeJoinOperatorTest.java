...
public class TemporalProcessTimeJoinOperatorTest extends TemporalTimeJoinOperatorTestBase {
	...
	private final RowDataTypeInfo rowType = new RowDataTypeInfo(
		new BigIntType(),
		new VarCharType(VarCharType.MAX_LENGTH));
	private final TypeInformation<RowData> keyType = new RowDataTypeInfo();
	private final BinaryRowDataKeySelector keySelector = new BinaryRowDataKeySelector(
		new int[]{keyIdx},
		rowType.getLogicalTypes());
	private final RowDataTypeInfo outputRowType = new RowDataTypeInfo(
		new BigIntType(),
		new VarCharType(VarCharType.MAX_LENGTH),
		new BigIntType(),
		new VarCharType(VarCharType.MAX_LENGTH));
	private final RowDataHarnessAssertor assertor = new RowDataHarnessAssertor(outputRowType.getFieldTypes());
	...
	private KeyedTwoInputStreamOperatorTestHarness<RowData, RowData, RowData, RowData> createTestHarness(
		TemporalProcessTimeJoinOperator temporalJoinOperator) throws Exception {
