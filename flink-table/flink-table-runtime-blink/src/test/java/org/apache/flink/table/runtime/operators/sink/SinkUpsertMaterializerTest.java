...
public class SinkUpsertMaterializerTest {
	...
	private final StateTtlConfig ttlConfig = StateTtlConfig.newBuilder(Time.seconds(1)).build();
	private final LogicalType[] types = new LogicalType[] {new IntType(), new VarCharType()};
	private final RowDataSerializer serializer = new RowDataSerializer(new ExecutionConfig(), types);
	private final RowDataKeySelector keySelector =
			new BinaryRowDataKeySelector(new int[0], types);
	...
	private void shouldEmit(
			OneInputStreamOperatorTestHarness<RowData, RowData> harness, StreamRecord<RowData> expected) {
		assertEquals(Collections.singletonList(expected.getValue()), getEmittedRows(harness));
	}
