...
public class SqlWatermark extends SqlCall {
	...
	private SqlNumericLiteral maxWait;

	public SqlWatermark(SqlIdentifier watermarkName, SqlIdentifier columnName, SqlNode functionCall, SqlNumericLiteral maxWait, SqlParserPos pos) {
		super(pos);
	...
	public void unparse(SqlWriter writer, int leftPrec, int rightPrec) {
		...
		if (maxWait != null) {
			writer.keyword("WAIT");
			maxWait.unparse(writer, leftPrec, rightPrec);
		}
	}
	...
	public String getSimpleWatermarkNameOrDefault(String defaultName) {
		if (watermarkName != null) {
			return watermarkName.getSimple();
		} else {
			return defaultName;
		}
	}
	...
	public long getMaxWait() throws SqlParseException {
		if (maxWait == null) {
			return 0L;
		}
		String offsetStr = maxWait.toValue();
		long offset;
		try {
			offset = Long.valueOf(offsetStr);
		} catch (NumberFormatException e) {
			throw new SqlParseException(
				maxWait.getParserPosition(),
				"The argument of 'wait' must be an integer, but is " + offsetStr,
				e);
		}
		return offset;
	}
