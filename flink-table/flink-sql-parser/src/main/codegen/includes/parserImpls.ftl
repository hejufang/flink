...
/**
* Support SQL call: SHOW TABLES FROM [catalog.] database
*/
...
}
{
    ...
    { pos = getPos(); }
    [
        ( <FROM> { preposition = "FROM"; } | <IN> { preposition = "IN"; } )
        { pos = getPos(); }
        databaseName = CompoundIdentifier()
    ]
    [
    ...
    {
        return new SqlShowTables(pos, preposition, databaseName, notLike, likeLiteral);
    }
}
