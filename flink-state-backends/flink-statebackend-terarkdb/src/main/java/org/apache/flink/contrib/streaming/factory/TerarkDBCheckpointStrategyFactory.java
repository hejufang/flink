...
import org.apache.flink.contrib.streaming.state.snapshot.AbstractCheckpointStrategyFactory;
...
/**
 * Create a Checkpoint Strategy for Terarkdb that supports WAL mode.
 */
public class TerarkDBCheckpointStrategyFactory extends AbstractCheckpointStrategyFactory {

	private final boolean enableWal;

	public TerarkDBCheckpointStrategyFactory(boolean enableWal) {
	...
	protected <K> RocksDBSnapshotStrategyBase<K> initializeCheckpointStrategy(
		...
		if (enableIncrementalCheckpointing) {
			...
			if (injectedBeforeTakeDBNativeCheckpoint == null){
				checkpointSnapshotStrategy = new TerarkDBIncrementalSnapshotStrategy<>(
						db,
				...
				checkpointSnapshotStrategy = new TerarkDBIncrementalSnapshotStrategy<K>(
						db,
						rocksDBResourceGuard,
						keySerializer,
						kvStateInformation,
