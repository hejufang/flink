...
public class RocksDBStateMisuseOptionTest {
	...
	public void testMisuseOptimizePointLookupWithMapState() throws Exception {
		TerarkDBStateBackend terarkDBStateBackend = createStateBackendWithOptimizePointLookup();
		RocksDBKeyedStateBackend<Integer> keyedStateBackend =
			createKeyedStateBackend(terarkDBStateBackend, new MockEnvironmentBuilder().build(), IntSerializer.INSTANCE);
		try {
	...
	private TerarkDBStateBackend createStateBackendWithOptimizePointLookup() throws IOException {
		TerarkDBStateBackend terarkDBStateBackend = new TerarkDBStateBackend(tempFolder.newFolder().toURI().toString(), true);
		terarkDBStateBackend.setPriorityQueueStateType(RocksDBStateBackend.PriorityQueueStateType.ROCKSDB);
		terarkDBStateBackend.setRocksDBOptions(new RocksDBOptionsFactory() {
