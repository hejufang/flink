...
import org.terarkdb.BlockBasedTableConfig;
import org.terarkdb.ColumnFamilyOptions;
import org.terarkdb.CompactionStyle;
import org.terarkdb.CompressionType;
import org.terarkdb.DBOptions;
import org.terarkdb.InfoLogLevel;
...
public class RocksDBStateBackendConfigTest {
	...
	public void testDefaultsInSync() throws Exception {
		...
		TerarkDBStateBackend backend = new TerarkDBStateBackend(tempFolder.newFolder().toURI().toString());
	...
	public void testSetDbPath() throws Exception {
		final TerarkDBStateBackend rocksDbBackend = new TerarkDBStateBackend(tempFolder.newFolder().toURI().toString());
	...
	public void testConfigureTimerService() throws Exception {
		...
		assertEquals("state.backend.rocksdb.timer-service.factory", RocksDBOptions.TIMER_SERVICE_FACTORY.key());
		...
		TerarkDBStateBackend rocksDbBackend = new TerarkDBStateBackend(tempFolder.newFolder().toURI().toString());

		RocksDBKeyedStateBackend<Integer> keyedBackend = createKeyedStateBackend(rocksDbBackend, env, IntSerializer.INSTANCE);
		assertEquals(RocksDBPriorityQueueSetFactory.class, keyedBackend.getPriorityQueueFactory().getClass());
		keyedBackend.dispose();
	...
	public void testConfigureTimerServiceLoadingFromApplication() throws Exception {
		...
		final TerarkDBStateBackend backend = new TerarkDBStateBackend(tempFolder.newFolder().toURI().toString());
		...
		final TerarkDBStateBackend configuredRocksDBStateBackend = backend.configure(
			configFromConfFile,
	...
	public void testSetEmptyPaths() throws Exception {
		...
		TerarkDBStateBackend rocksDbBackend = new TerarkDBStateBackend(checkpointPath);
	...
	public void testPredefinedOptions() throws Exception {
		...
		rocksDbBackend = new TerarkDBStateBackend(checkpointPath);
	...
	public void testRocksDbReconfigurationCopiesExistingValues() throws Exception {
		...
		final TerarkDBStateBackend original = new TerarkDBStateBackend(checkpointBackend, TernaryBoolean.fromBoolean(incremental));
		...
		TerarkDBStateBackend copy = original.configure(new Configuration(), Thread.currentThread().getContextClassLoader());
	...
	public void testCallsForwardedToNonPartitionedBackend() throws Exception {
		...
		TerarkDBStateBackend rocksDbBackend = new TerarkDBStateBackend(storageBackend);
	...
	private void verifyIllegalArgument(
		TerarkDBStateBackend stateBackend,
		String key,
