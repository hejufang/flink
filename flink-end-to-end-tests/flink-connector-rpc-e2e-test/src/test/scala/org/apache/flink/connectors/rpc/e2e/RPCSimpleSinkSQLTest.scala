...
import org.apache.flink.connectors.rpc.thriftexample.{BatchDimMiddlewareService, SimpleDimMiddlewareService}
...
class RPCSimpleSinkSQLTest extends StreamingTestBase {
  private val LOG = LoggerFactory.getLogger(classOf[RPCSimpleSinkSQLTest])
  ...
  val serviceImpl = new SimpleSinkServiceImplTest()
  ...
  override def before(): Unit ={
    ...
    server = new TSimpleServer(new TServer.Args(serverTransport)
      ...
      .processor(
        new SimpleDimMiddlewareService.Processor[SimpleDimMiddlewareService.Iface](serviceImpl)))
  ...
  def testSimple(): Unit = {
    tEnv.sqlUpdate(
      s"""
        ...
        |'org.apache.flink.connectors.rpc.thriftexample.SimpleDimMiddlewareService',
        |'connector.thrift-method' = 'GetDimInfos',
        |'connector.test.host-port' = '127.0.0.1:${thriftPort}'
        |)
        |""".stripMargin)
    val streamTable = env.fromCollection(data)(dataRowType)
      .toTable(tEnv, 'adId, 'dimensionList)
