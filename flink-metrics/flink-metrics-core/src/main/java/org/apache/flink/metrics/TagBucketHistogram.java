 ...
 * Histogram with tags.
 *
 * <p>Note: it shouldn't be used as normal histogram, {@link #update(long)}
 * and {@link #getStatistics()} are not supposed to be used.
 * TODO：rewrite it
 */
public class TagBucketHistogram implements Histogram {
	...
	public static final long[] DEFAULT_BUCKETS = {
		0L, 60L, 120L, 180L, 240L, 300L,
		360L, 420L, 480L, 540L, 600L,
		660L, 720L, 780L, 840L, 900L
	};
	...
	public void update(long value) {
		tagHistograms.compute(EMPTY_TAG, (k, v) -> {
			if (v == null) {
	...
	public void update(long value, Map<String, String> tags) {
		...
		tagHistograms.compute(tags, (k, v) -> {
			if (v == null) {
				v = new BucketHistogram(buckets, quantiles);
			}
			v.update(value);
	...
	public HistogramStatistics getStatistics() {
		return tagHistograms.containsKey(EMPTY_TAG) ? tagHistograms.get(EMPTY_TAG).getStatistics() : null;
	}
	 ...
	 *
	 *  <p>The histogram is not thread safe, no other thread should update it when {@link #getStatistics()} is called.
	 *
	 *  <p>Note that the buckets should start from zero and every bucket is left point inclusive and right
	 *  point exclusive.
	 *
	 *
	 *  <p>For example, the buckets array is [0, 3, 5, 10] and the interval it represents is:
	 *  [0, 3), [3, 5), [5, 10), [10, +infinity).
	 */
	static class BucketHistogram implements Histogram {
		 ...
		 *
		 * <p>Note: buckets should be sorted and leading number should be zero and
		 * quantiles should also be sorted.
		 *
		 ...
		 * Make sure no other thread update it when the method is called.
		 *
