...
public class TestOpentsdbReporter {
	...
	public void testGetMetricNameAndTags1() {
		...
		Tuple<String, String> expect = new Tuple<>(
			"jobmanager.Status.JVM.Memory.Direct.TotalCapacity",
			"jobname=" + reporter.getJobName() + "|region=" + reporter.getRegion() +
			"|hostname=n8_159_232.byted.org");
	...
	public void testGetMetricNameAndTags2() {
		...
		Tuple<String, String> expect = new Tuple<>(
			"taskmanager.Streaming_WordCount.Keyed_Aggregation.latency",
			"jobname=" + reporter.getJobName() + "|region=" + reporter.getRegion() +
			"|hostname=n8_159_070|tmid=554a025ffcd1bb5845bc58152d3e4355|taskid=1");
		Tuple<String, String> actual = reporter.getMetricNameAndTags(key);
		Assert.assertEquals(expect.x, actual.x);
		Assert.assertEquals(expect.y, actual.y);
	}
	...
	public void testKafkaConsumerMetrics() {
		...
		Tuple<String, String> expect = new Tuple<>(
			"taskmanager.user.Source_mySource.KafkaConsumer.currentOffsetsRate",
			"jobname=" + reporter.getJobName() + "|region=" + reporter.getRegion() +
			"|hostname=n8_132_204|tmid=container_e486_1569378662011_14972_01_000002|taskid=2|" +
			"topic=data_flink_test|partition=6");
		Tuple<String, String> actual = reporter.getMetricNameAndTags(key);
		Assert.assertEquals(expect.x, actual.x);
		Assert.assertEquals(expect.y, actual.y);
	}
